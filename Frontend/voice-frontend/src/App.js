...
function App() {
  ...
  const handleModeChange = (e) => {
    if (uploading) return; // ❌ prevent switching during upload
  ...
  const handleFileSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/transcribe/", // 🔥 UPDATE THIS
        formData
      );

      setLines([response.data.text || "No text found"]);
    } catch (error) {
      console.error(error);
  ...
  return (
    <div style={{ textAlign: "center", marginTop: "50px" }}>
      ...
      <div>
        <label>
          <input
            ...
            value="live"
            checked={mode === "live"}
            disabled={true} // 🔥 fully disable live
          />
          Live Voice (Disabled)
        </label>
      ...
      <div>
        ...
        <button onClick={handleFileSubmit} disabled={uploading}>
      ...
      <button
        onClick={clearText}
        disabled={lines.length === 0 && !loadingText}
      >
        Clear Text
      </button>
    </div>
